...
// i thought he was using random

// const randomIndex = (arr) => Math.floor(Math.random()*arr.length)
// const getRandomElement = (arr) => arr[randomIndex(arr)]
const templateText = (paragraph) => `<p class="result">${paragraph}</p>`

const form = document.querySelector("form")
form.addEventListener("submit", (e) => {
  e.preventDefault()
  
  const loremResult = document.querySelector(".lorem-text")
  const amountInput = document.getElementById("amount")

  const value = (Number(amountInput.value) <= 0 || Number(amountInput.value) > text.length) ? 1 : Number(amountInput.value)
  const templateTexts = text.slice(0, value).map(item => templateText(item))
  loremResult.innerHTML = templateTexts.join("")
  amountInput.value = ""
})
